...
# Get scalar sources
info_black('Getting ScalarValues')
...
info_black('Preassembling and allocating')
...
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    vars().update( update_body_force(**vars()) )
    assert(isinstance(f, Coefficient))
    b0 = dict((ui, assemble(v * f[i] * dx)) for i, ui in enumerate(u_components))
    ...
    if tstep % print_intermediate_info == 0:
        toc = tx.stop()
        info_black( 't = %1.2e, t-step = %1d, T = %1.2e'%(t, tstep, T))
        info_black('Total computing time on previous %1d t-steps = %1dh%1dm%1ds'%(
            print_intermediate_info, toc/60**2, (toc%60**2)/60, (toc%60**2)%60))
        #(TimingClear.clear, [TimingType.wall])
        tx.start()
...
toc = total_timer.elapsed()[0]
info_black('Total computing time =  %1dh%1dm%1ds'%( toc/60**2, (toc%60**2)/60, (toc%60**2)%60) )
oasis_memory('Final memory use ')
total_initial_dolfin_memory = MPI.sum(MPI.comm_world, initial_memory_use)
info_black('Memory use for importing dolfin = {} MB (RSS)'.format(
    total_initial_dolfin_memory))
info_black('Total memory use of solver = ' +
